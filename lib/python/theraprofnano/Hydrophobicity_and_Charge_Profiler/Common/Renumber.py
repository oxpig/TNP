...
def save_colored(filename,chains,mapping,out_file,id_map):
	...
	for line in f.readlines():
		if line[0:4]=="ATOM":
			...
			except KeyError:#This means that the structure is bigger, having constant region etc. Ignore.
				continue
...
#CDR Definitions
definitions = {		
		"kabat" : {"H1" : ["H31", "H32", "H33", "H34", "H35"] ,
				  "H2" : ["H50", "H51", "H52", "H53", "H54", "H55", "H56", "H57", "H58", "H59", "H60", "H61", "H62", "H63", "H64", "H65"] ,
				  "H3" : ["H95", "H96", "H97", "H98", "H99", "H100", "H101", "H102"]}
		,
		"chothia" : {"H1" : ["H26", "H27", "H28", "H29", "H30", "H31", "H32"], 
				  "H2" : ["H52", "H53", "H54", "H55", "H56"] ,
				  "H3" : ["H95", "H96", "H97", "H98", "H99", "H100", "H101", "H102"]} 
		,
		"contact" : {"H1" : ["H30", "H31", "H32", "H33", "H34", "H35"], 
		"H2" : ["H47", "H48", "H49", "H50", "H51", "H52", "H53", "H54", "H55", "H56", "H57", "H58"] ,
		"H3" : ["H93", "H94", "H95", "H96", "H97", "H98", "H99", "H100", "H101"] }
		}

#If we want to constrain as a function import
#Input:
#input_file: the pdb where the nanobody is.
#chains: specify the heavy chain.
#deff: which definition to use: chothia, kabat, imgt etc.
#out_dir: directory where the files should be saved.
def constrain_nanobody(input_file,chains,deff,out_file):
	#All the temporary files are stored here
	tempfold = create_temp_folder()
	cwd = os.getcwd()

	filename = check_file(input_file)
	...
	for chain in chains:
		...
		zipped = list(zip(resnames,ch_map))
		for elem in zipped:
		
			cdr = is_CDR(stripChothia(elem[1][0]),deff)
			...
			if chain_maps[chain][elem[0]] != elem[1][1]:
				...
				print("Something wrong in the mapping:",out_file)
				print(chain_maps[chain][elem[0]])
				print(elem[1][1])				
				cleanup(tempfold)
				return				
	
	#Save the pdb file with colored CDRs
	#write_data(out_dir+"/full_info.txt",cdr_detailed)
	#write_data(out_dir+"/paratope.txt",output_cdr)
	#write_data(out_dir+"/framework.txt",output_frame)
	save_colored(filename,chains,cdr_map,out_file,id_map)

	cleanup(tempfold)


#If we want to use this file as a command line tool.
if __name__ == '__main__':
	...
	##Read In options##
	#Parse the options
	usage = "USAGE: python Framer.py --f FILE --c CHAINS"
	parser = OptionParser(usage=usage)

	#Single file mode
	parser.add_option("--f",help="Nanobody file location", dest="file")
	parser.add_option("--c",help="Nanobody chain to constrain, e.g. --c ABCD", dest="chains")
